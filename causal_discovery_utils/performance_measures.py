...
from causal_discovery_algs.fci import LearnStructFCI  # used here to find the learned PAG
...
def find_true_pag(true_dag, true_observed_set):
    perfect_ci_test = DSep(true_dag=true_dag)
    fci_true = LearnStructFCI(nodes_set=true_observed_set, ci_test=perfect_ci_test)
    fci_true.learn_structure()  # find the PAG using the FCI algorithm
    return fci_true.graph
